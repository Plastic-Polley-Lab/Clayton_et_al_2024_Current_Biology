function RLF_analysis_preprocess_tosca(path, region)
% cd(path)
mydir  = pwd;
file = dir('*-adj.mat');
if isempty(file)
    sprintf('No manually adjustment')
    error('You should manually check the spike quality')
...
for j = 1:length(file)
    ...
    load(filename)
    check_responses(filename, spikedata, region)
end


function check_responses(filename, spikedata, region)
keep = find([spikedata.keep] ==1);
idcs2 = strfind(filename,'_');
animalID = filename(1:idcs2(1)-1);
...
for i = 1:length(keep)
    indx = keep(i);
    isi = diff(spikedata(indx).spiketimes);
    ...
    time = spikedata(1).psth.timepoint;
    baseline_window = find(time > -base_window/1000 & time < 0);
    evoke_window    = find(time > 0 & time < test_window/1000);
    ...
    if isnan(spikedata(indx).refra_violation)
        ...
        scmatrix = spikedata(indx).psth.scmatrix;
...
for i = 1:length(keep)
    try
        neuron_num = keep(i);
        [spikedata(neuron_num).spls,  spikedata(neuron_num).rlf, spikedata(neuron_num).spont, spikedata(neuron_num).psth_summary] = rlf_SU_analysis_tosca(neuron_num, spikedata, analysis_window);
...
save([animalID, '_rlf_responses.mat'],'spikedata')
