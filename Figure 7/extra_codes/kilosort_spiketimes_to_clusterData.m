function kilosort_spiketimes_to_clusterData(analdir,chanmap_file)
...
%         else
        if j==length(timepoints)
            indxs = find(spikes> timepoints(length(timepoints)));
            clusterData.run(j).spiketimes = (spikes(indxs) - timepoints(length(timepoints))).*1000;
            
            indxs = find(store_ticks> timepoints(length(timepoints)));
            clusterData.run(j).ticks = (store_ticks(indxs) - timepoints(length(timepoints))).*1000;
        else
            indxs = find(spikes> timepoints(j) & spikes <= timepoints(j+1));
            clusterData.run(j).spiketimes = (spikes(indxs) -  timepoints(j)).*1000;
            
            indxs = find(store_ticks> timepoints(j) & store_ticks <= timepoints(j+1));
            clusterData.run(j).ticks = (store_ticks(indxs) -  timepoints(j)).*1000;
        end
...
%         file)
        ...
        stimData.innerIndexes = inner_indexes;
    ...
    filename = [analdir '_SpikeData/' split_strings{end-3} '-' split_strings{end-2} '-' split_strings{end-1} '-' sprintf('cluster_%d',clus_id) '.mat'];
