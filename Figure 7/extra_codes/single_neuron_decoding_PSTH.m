...
%%
function options = single_neuron_decoding_PSTH(data, options)
...
%%
% here I'll leave-one-out from each class as testing set and use the rest
% as training sets. updated by Ke on 10/26/2020 to introduce some randomly,
% before all the testing index for each trial types are the same

testing_indx = 1:options.mintrials; % the index of testing_index
% add by Ke to have different testing index for
% each class
% for i = 1:nev
...
training_indx  = zeros(options.mintrials-1, length(testing_indx));
for i = 1:length(testing_indx)
    training_indx(:,i) = setdiff(1:options.mintrials, testing_indx(i))';
end

% start the decoding process
iter = 1;
for i = 1:length(testing_indx)
    % get the template of each class and the testing data for each class
    for j = 1: options.nclass
        temp.(trig{j}) = mean(psth.(trig{j}).scmatrix(training_indx(:,i), options.windows(1):options.windows(2)),1);
        test.(trig{j}) = psth.(trig{j}).scmatrix(testing_indx(i), options.windows(1):options.windows(2));
        if smooth_set
...
% plotconfusion(options.trueClass, options.Classified)
